...
class main:
	...
	def sendRequest(self, scraper, headers, type):
		def Cash(scraper, headers, type):
		...
		if type == 0:
			...
			while True:
				...
				threading.Thread(target=Bet, args=(scraper, headers, type,)).start()
				threading.Thread(target=Cash, args=(scraper, headers, type,)).start()
	...
	def sendBets(self): # Actually compare the user's chances of winning and place the bets
		...
		while True:
			...
			time.sleep(2.5)
			self.exploded = False
			threading.Thread(target=self.sendRequest, args=(scraper, headers, 1)).start()
			# time.sleep(3)
			# threading.Thread(target=self.sendRequest, args=(scraper, headers, 0)).start()
